...
import { MemoryFixedWindowLimiter } from './ratelimit/memoryFixedWindow.js';
import { MongoFixedWindowLimiter } from './ratelimit/mongoFixedWindow.js';
...
async function main(): Promise<void> {
  ...
  const rateLimiter =
    env.RATE_LIMIT_STORE === 'memory'
      ? new MemoryFixedWindowLimiter(env.RATE_LIMIT_PER_MINUTE)
      : new MongoFixedWindowLimiter(cols, env.RATE_LIMIT_PER_MINUTE);
